...
public class Timer {
    ...
    public <T, U> double repeat(int n, Supplier<T> supplier, Function<T, U> function, UnaryOperator<T> preFunction, Consumer<U> postFunction) {
        ...
        for (int i = 0; i < n; i++) {
            pause(); // stop before supplier gets
            T time = supplier.get(); // value of supplier stored in T
            if (preFunction != null) {
                time = preFunction.apply(time); // applying preFunction
            } // checking whether preFunction is null
            resume(); // resuming the timer
            U u = function.apply(time); // changing T to U and then timing it
            pauseAndLap(); 
            if (postFunction != null) { // analyzing whether postFunction is null
                postFunction.accept(u);
            }
            resume(); // continue the timer
        }
        pause();
        return meanLapTime(); // returning time 
